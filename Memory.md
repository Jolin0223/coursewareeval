...
- 2026-07-11 课件生成提示词测评的测试分支排查：本地直连 `https://box.xdf.cn/kpm-api/skill/material-design` 能通过同一套 HMAC 签名，说明接口路径和算法本身可用；Worker 侧需防止 Cloudflare 环境变量手工录入时带空格/换行导致签名失败。`buildKpmAuthHeaders()` 现在会 trim `KPM_APP_SECRET` 与 `KPM_APP_ID`，`/api/version` 返回 `trimsKpmEnv`，诊断接口返回原始长度和 trim 后长度。生成失败提示也会带 Worker buildId，用于区分新版错误与浏览器本地旧失败记录。
- 2026-07-11 继续排查：测试分支 Worker 调 `/kpm-api/skill/material-design` 三种请求形态均返回上游 `566 text/html`，但同样 HMAC、同样 5KB 量级 `content` 从本机直连 `https://box.xdf.cn/kpm-api/skill/material-design` 返回 `200 text/event-stream`。这说明更像是 KPM 网关对 Cloudflare Worker 出口或请求头做了限制，不是提示词长度或接口路径错误。Worker 新增 `https-python-headers` fallback，显式使用类似 Python 脚本的 `User-Agent`；如果仍 566，下一步应改为非 Cloudflare 的后端执行器/代理，而不是继续在前端绕。
- 2026-07-11 用户确认 `https-python-headers` 仍返回 566，已实现独立 `generation-runner`。网站按钮仍请求 `/api/generation-eval/run-prompt-version`，Worker 先校验 Supabase 管理员；如果 Cloudflare 环境变量 `GENERATION_RUNNER_URL` 存在，则把任务流式代理给 runner，由 runner 使用普通 Python 网络环境调用 `material-design` 和 `material-create`。runner 位于 `generation-runner/server.py`，只从环境变量读取 `KPM_APP_SECRET`，不要把密钥提交到仓库。测试环境要真正跑通还需要部署一个公网 runner，并在 Cloudflare Pages 测试环境配置 `GENERATION_RUNNER_URL`，可选配置 `GENERATION_RUNNER_TOKEN`。
...
- `课件生成效果` 不要再使用“上方快捷用例列表 + 下方单个详情”的布局。固定测试集按当前学科、年级筛选结果直接向下平铺；每个课件内部左侧线上现状、右侧提示词版本效果并列展示，确保一眼能看出同一课件的两个效果差异。
- `课件生成效果` 的线上现状/提示词版本双栏对比在普通桌面宽度就要并排展示，不能只在超宽屏断点才生效；窄移动端再自然堆叠。
- `课件生成效果` 顶部不要放解释型“评分视角/平铺评审”大模块；只保留对当前测评有判断价值的薄结论卡。评分口径和录入对象属于评分操作，应放在每张评分卡的四维评分区域内。
- `课件生成效果` 的线上现状卡只能记录基线事实状态，不能出现“优于线上/持平/不如线上”这类实验组结论；提示词最新版本卡才提供比较结论与“生成下一版提示词草稿”入口，草稿生成后只供查看/复制审核，不自动入库或覆盖正式版本。
- `课件生成效果` 切换评分显示口径或滑杆写入对象时，只能局部刷新评分区、结论卡和总览表，不能重建 iframe 预览导致课件封面刷新。线上现状卡底部不需要任何结论按钮。
- 2026-08-11 起，旧12条课件只保留为历史实验，不再作为常规测试集。`课件生成效果` 固定读取 `0312-20260813系统提示词效果固定测试集/requests.json` 的28条自动用例，逐条原样调用 `box-pipeline/scripts/generate.py`，固定使用 `https://box.test.xdf.cn`，只生成，不评审、不迭代、不上架；另6条音频/录音/口语用例仅由Jolin手动生成。网站清空旧12条种子和旧评分存储，左侧线上结果先占位，右侧回填测试环境结果；提供学科、年级筛选，每条展示用户输入原文、主交互、重点验收，并固定提示测试环境预览需在公司内网下打开。历史课件HTML和旧实验归档不修改。
- 2026-08-11 固定自动集首次实跑完成：28/28均获得测试环境预览URL和ZIP，评审0、迭代0、上架0；T17、T18首次未收到`finalResult`，断点重跑后成功。生成证据保存在 `0312-20260813系统提示词效果固定测试集/runs/20260811-box-test-v1.1`，网站总口径为34条（28自动成功 + 6手动待生成），线上34条均为空占位。
- 固定测试集批量生成永远只允许使用 `box-pipeline` skill 和本地脚本调用 Box 接口，禁止通过浏览器发起生成。浏览器只可用于查看或验证已生成结果。批量任务默认错峰并发提交，失败发生在 create 阶段且已有 `conversationId` 时必须复用原会话续跑，避免重复方案设计。
- 2026-08-12 V1.5 固定自动集按每6秒启动1条、28路并发完成，最终28/28成功，评审0、迭代0、上架0。T25首次create返回500，原`conversationId`多次续跑仍未进入生成步骤，最终只为T25新建方案会话后成功；以后遵循“先复用会话，确认原会话持续不可用后只重建失败用例”的恢复顺序，不重跑整批。
- 2026-08-12 V1.6 固定自动集按每20秒启动1条、28路并发执行，28条在约9分钟内全部提交，最终28/28成功，评审0、迭代0、上架0。T23、T25、T27、T28首次create在同一时刻失败，复用各自原`conversationId`续跑后全部成功，未重建设计会话。后续默认采用“28路并发、每20秒错峰、10分钟内提交完”的节奏，避免3分钟内集中请求可能造成的图片资源异常；仍禁止通过浏览器发起生成。
- V1.6 生成后追加了本地引用与最终ZIP资源完整性检查：27/28未发现静态资源缺失；T08“拨时钟”在完全独立重建方案并单独生成两次后，最终ZIP仍引用但未打包`hour_hand.png`和`minute_hand.png`，属于该需求当前生成结果的稳定缺陷，不是批量并发造成，作为固定测试真实问题保留。T13、T14首次结果的资源缺失经单独重建后消失。批量脚本支持通过`--case-id Txx --restart-case-id Txx`强制重建已成功但资源不完整的指定用例，不重跑整批。
- `课件生成效果` 右侧实验组有多个提示词版本时不要平铺按钮，使用自绘下拉版本选择器，默认优先选中 label 包含“最新版”的版本。评分口径按钮必须小字号、单行展示；主要问题和当前版本亮点输入框的 placeholder 不能继承加粗。
