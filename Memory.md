...
- `跑最新版效果` 不能只 fire-and-forget 素材修改，否则 Cloudflare 主请求返回后可能终止后台调用，实际只完成一键同款。正确的临时方案是：同步等待 `material-modify` 返回响应头/HTTP 200，立即 cancel SSE body，不读取完整流；这样能确认风格提示词已被素材修改接口接收，同时避免等待整条生成流。
