...
- `课件生成效果` 不要再使用“上方快捷用例列表 + 下方单个详情”的布局。提示词版本对比应平铺 12 个课件，用户直接向下滚动；每个课件内部左侧线上现状、右侧提示词版本效果并列展示，确保一眼能看出同一课件的两个效果差异。
...
- 2026-08-06 起，`课件生成效果` 的 12 条实验需求固定使用用户原文，不再组装 `系统提示词 + 用户需求 + 测评说明`。Box 生成固定走 `https://box.test.xdf.cn/kpm-api/skill/material-design` 和 `/material-create`，不得调用 modify、pipeline、一键同款或上架。网站左侧 `baseline` 是不可变对照组；只有 12 条都拿到有效 `fileUrl`、`pushUrl` 并完成评测后，才把新版本追加为右侧实验组最新版，失败结果不写站点。2026-08-06 全量实测成功 0/12：12 条均有 `conversationId`，但 create SSE 未返回 `finalResult`，因此未评测、未改网站、未部署。修复后应复用现有会话续跑 create，证据见 `Obsidian Vault/03-AI互动课件制作/0306-Box测试环境12课件评测-20260806/generation-summary.json`。
